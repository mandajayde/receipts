...
# The same gap: drafting a receipt for a pull request this agent did not write.
author=(pr.get('user') or {}).get('login','').lower(); acct=(me.get('account') or '').lower()
if not acct: sys.exit(f"agents/{a.agent}.json declares no account, so authorship cannot be checked")
if author!=acct: sys.exit(f'{owner}/{repo}#{num} was opened by {author or "nobody recorded"}, not by {acct}; a receipt is for work this agent did')
