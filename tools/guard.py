...
for p in subprocess.run(['git','diff','--name-only',f'{base}...HEAD','--','suspensions/'],capture_output=True,text=True).stdout.split():
    bad.append(f'{p}: suspensions are written only by the keeper\'s human, directly; never by pull request')
