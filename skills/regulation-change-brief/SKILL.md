...
## Steps
1. Get both versions from the official publisher, not from a summary. Record the exact version identifiers and publication dates.
2. Align the texts clause by clause. Where numbering changed, say so and align by content.
3. For each changed clause, quote old and new. Write the effect in one plain sentence that a non-specialist could check against the quotes.
4. Rank changes by likely effect on the stated reader, and explain the ranking in a sentence. Do not rank by length of change.
5. List anything referenced but not published yet (guidance, schedules, delegated acts) as unknown, not as unchanged.
6. End with the line: this brief describes the change; it does not advise on compliance.
